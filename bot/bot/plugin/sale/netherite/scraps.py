...
@plugin.include
@cmd.cmd("ломов",
         desc="Продажа незеритовых ломов",
...
class Command(cmd.Command):
    ...
    async def cb(self, ctx: crescent.Context) -> None:
        ...
        ratio = (config.get("plugins").get("ratio").get("purchase").get(
            "netherite").get("scraps")) // 2

        title = f"{emoji.Emoji.Netherite.SCRAP} Продажа незеритовых ломов"

        async def saleNetheriteScraps(msgCtx: flare.MessageContext,
            ...
            try:
                ...
                await db.dec(id_, "netheriteScrap", netheriteScrapQuantity)
        ...
        desc = code.code(f"1 незеритовый лом к {ratio} монетам")

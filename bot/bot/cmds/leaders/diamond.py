...
@plugin.include
@cmd.cmd("алмазов", desc="Лидеры алмазов", period=periods.period, group=groups.group)
class Command(cmd.Command):
    async def cb(self, ctx: crescent.Context) -> None:
        ...
        desc = "\n".join(
            [
                f"{emojis[pos]} **#{pos}** <@{user.id}> {d.decorate(h.humanize(user.diamond))}"
                for pos, user in enumerate(users, start=1)
            ]
        )

        await ctx.respond(
            embeds=embed.embed("diamond", title="leaders-diamond", desc=desc)
        )

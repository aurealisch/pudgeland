...
class Applications(flare.Modal, title=title):
    ...
    tellUsAboutYourself: flare.TextInput = flare.TextInput(
        "Расскажите о себе (чем больше тем лучше)", placeholder="У меня амнезия"
    )
    whatContentDoYouPreferForMinecraft: flare.TextInput = flare.TextInput(
        "Какой контент предпочитаете по Minecraft", placeholder="Я не играю в игры..."
    )
    ...
    async def callback(self, modalCtx: flare.ModalContext) -> None:
        ...
        async def cancel(msgCtx: flare.MessageContext) -> None:
            class Cancellation(flare.Modal, title=title):
                ...
                async def callback(self, modalCtx: flare.ModalContext) -> None:
                    ...
                    embeds.append(embed.embed("reject", title=title, desc=desc))
        ...
        guildTextChannel: hikari.GuildTextChannel = (
            await modalCtx.app.rest.fetch_channel(1044710321815818302)
        )

        msg: hikari.Message = await guildTextChannel.send(component=comp, embeds=embeds)

        desc = "Заявка отправлена на рассмотрение!"

        await modalCtx.respond(
            flags=flags, embed=embed.embed("applications", title=title, desc=desc)
        )
